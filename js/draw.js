...
// מטמון גרדיאנטים: כל גוף מצויר במערכת צירים מקומית, כך שאותו גרדיאנט משמש בכל פריים.
// (CanvasGradient מתפרש לפי הטרנספורמציה בזמן המילוי — לכן אפשר להזיז אותו עם translate/rotate.)
const GRAD = new Map();
function grad(key, make) {
  let g = GRAD.get(key);
  if (!g) { if (GRAD.size > 800) GRAD.clear(); g = make(); GRAD.set(key, g); }
  return g;
}
const gctx = document.createElement('canvas').getContext('2d');
function linG(key, x0, y0, x1, y1, stops) { return grad(key, () => { const g = gctx.createLinearGradient(x0, y0, x1, y1); stops.forEach(([o, c]) => g.addColorStop(o, c)); return g; }); }
function radG(key, x0, y0, r0, x1, y1, r1, stops) { return grad(key, () => { const g = gctx.createRadialGradient(x0, y0, r0, x1, y1, r1); stops.forEach(([o, c]) => g.addColorStop(o, c)); return g; }); }
// ממלא את הנתיב הנוכחי כשהגרדיאנט מוזז ל-(tx,ty) — הנתיב עצמו לא זז
function fillAt(ctx, tx, ty) { ctx.save(); ctx.translate(tx, ty); ctx.fill(); ctx.restore(); }

function taper(ctx, A, B, wA, wB, base, bulge, light = 0.22, dark = 0.4) {
  const dx = B[0] - A[0], dy = B[1] - A[1], L = Math.hypot(dx, dy) || 1, ang = Math.atan2(dy, dx);
  const W = Math.round(Math.max(wA, wB) * 1.15), f = Math.cos(ang) >= 0 ? 1 : -1; // הצד המואר תמיד למעלה
  ctx.fillStyle = linG(`t|${base}|${W}|${f}|${light}|${dark}`, 0, -f * W / 2, 0, f * W / 2,
    [[0, shade(base, light)], [0.28, base], [0.75, shade(base, -dark * 0.5)], [1, shade(base, -dark)]]);
  ctx.save(); ctx.translate(A[0], A[1]); ctx.rotate(ang);
  ctx.beginPath(); ctx.moveTo(0, -wA / 2); ctx.lineTo(L, -wB / 2); ctx.lineTo(L, wB / 2); ctx.lineTo(0, wA / 2); ctx.closePath(); ctx.fill();
  circle(ctx, 0, 0, wA / 2); circle(ctx, L, 0, wB / 2);
  if (bulge) { const [t, wf] = bulge; ell(ctx, L * t, 0, L * 0.34, lerp(wA, wB, t) * wf / 2); }
  ctx.restore();
}

// ---------- נעליים גבוהות ----------
function shoe(ctx, sx, c) {
  ctx.save(); ctx.translate(sx, 0);
  const x = 0, y = 0;
  ctx.fillStyle = linG('shoe|' + c, 0, -36, 6, 0, [[0, shade(c, 0.22)], [0.55, c], [1, shade(c, -0.35)]]);
  ctx.beginPath();
...
function drawShorts(ctx, c, by, pattern) {
  ctx.fillStyle = linG('shorts|' + c, -35, 0, 36, 0, [[0, shade(c, -0.42)], [0.3, shade(c, -0.12)], [0.58, shade(c, 0.28)], [0.7, c], [1, shade(c, -0.22)]]);
  shortsPath(ctx, by); ctx.fill();
  ...
  ctx.fillStyle = linG('wb|' + wb, 0, -139, 0, -128, [[0, shade(wb, 0.3)], [1, shade(wb, -0.25)]]);
  rr(ctx, -27, -139 + by, 54, 11, 3); fillAt(ctx, 0, by);
...
function drawTorso(ctx, skin, lx, by, p) {
  ...
  ctx.fillStyle = radG('torso|' + skin, 10, -178, 4, 2, -160, 64, [[0, shade(skin, 0.2)], [0.4, skin], [0.8, shade(skin, -0.2)], [1, shade(skin, -0.38)]]);
  torsoPath(ctx, lx, by); fillAt(ctx, lx, by);
...
function drawHead(ctx, look, hx, hy, p) {
  ...
  ctx.fillStyle = radG('head|' + skin, 9, -9, 2, 2, 2, r * 1.45, [[0, shade(skin, 0.2)], [0.45, skin], [0.85, shade(skin, -0.22)], [1, shade(skin, -0.38)]]);
  ctx.beginPath(); ctx.arc(hx, hy, r, 0, Math.PI * 2);
  ctx.ellipse(hx + 9, hy + 11, 13, 11, 0.2, 0, Math.PI * 2);
  // אף
  ctx.moveTo(hx + 18, hy - 7); ctx.quadraticCurveTo(hx + 27, hy + 3, hx + 25, hy + 6); ctx.lineTo(hx + 18, hy + 7); ctx.closePath();
  fillAt(ctx, hx, hy);
  ...
  if (d > 0.15) {
    ...
    const aq = Math.round(a * 10) / 10;
    ctx.fillStyle = radG('bruise|' + aq, 0, 0, 0, 0, 0, 9, [[0, `rgba(135,30,55,${0.45 * aq})`], [1, 'rgba(135,30,55,0)']]);
    ctx.beginPath(); ctx.ellipse(hx + 13, hy + 3, 9, 7, 0, 0, Math.PI * 2); fillAt(ctx, hx + 13, hy + 3);
    if (a > 0.35) {
...
function drawGlove(ctx, g, e, col) {
  ...
  ctx.fillStyle = linG('cuff|' + col, 0, -10, 0, 10, [[0, shade(col, 0.2)], [0.5, col], [1, shade(col, -0.45)]]);
  rr(ctx, -25, -9.5, 19, 19, 5); ctx.fill();
  ctx.fillStyle = 'rgba(255,255,255,.88)'; ctx.fillRect(-18, -9.5, 3, 19);
  // אגרוף
  ctx.fillStyle = radG('fist|' + col, 5, -7, 1, 0, 0, 19, [[0, shade(col, 0.42)], [0.4, col], [1, shade(col, -0.5)]]);
  ell(ctx, 1, 0, 16.5, 14.5);
...
function drawFighter(ctx, look, x, y, dir, pose, s) {
  ...
  const R = Math.round((58 + p.fall * 80) * s);
  ctx.fillStyle = radG('shadow|' + R, 0, 0, 0, 0, 0, R, [[0, 'rgba(0,0,0,.55)'], [0.6, 'rgba(0,0,0,.25)'], [1, 'rgba(0,0,0,0)']]);
  circle(ctx, 0, 0, R); ctx.restore();
  ctx.scale(dir * s, s);
...
// ===== הזירה =====
// מכשיר מגע = טלפון/טאבלט: מורידים אפקטים יקרים ורזולוציה כדי לשמור על קצב פריימים
...
function drawArenaFront(ctx, W, H) {
  const L = sceneLayers(ctx);
  blit(ctx, L.front);
  if (LOW_FX) return; // בטלפון מדלגים על הגרעין: מילוי מסך מלא בכל פריים יקר מדי
  ctx.save();
...
const Anims = new Set();
function sizeCanvas(c, cap = 2) {
  const dpr = Math.min(cap, window.devicePixelRatio || 1), w = c.clientWidth, h = c.clientHeight;
  if (!w || !h) return false;
